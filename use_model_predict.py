'''
使用训练好的model（虽然准确率不高），结合opencv完成人脸识别及表情预测，使用电脑摄像头完成时时的人脸检测
'''
import cv2
...
# 使用cv2自带的haar特征检测人脸位置，可能检测多张人脸，返回每个人脸的坐标位置
def detect_face(img):
    gray_img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    # 1.3这个参数可以试试调节为1.1，检测人脸的效果可能会更好
...
def draw_faces(img, faces, faces_emotion):
    color_index = 0
    for (x, y, w, h), emotion in zip(faces, faces_emotion):
        cv2.rectangle(img=img, pt1=(x, y), pt2=(x+w, y+h), color=color_list[color_index], thickness=2)
        cv2.putText(img, emotion, org=(int((x + w)/2), y), fontFace=font, fontScale=1,
